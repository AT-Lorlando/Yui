...
satellite/wakeword.py — fenêtre glissante de 16 frames d'embedding (~2s)
    │
...
satellite/main.py — démarre l'enregistrement + stream WebSocket
```
...
satellite/wakeword.py                 ← chargement ONNX + inférence OWW (sur le Pi)
satellite/main.py                     ← intégration dans la boucle satellite
satellite/ecosystem.config.js         ← --model, --threshold (env WAKEWORD_MODEL / WAKEWORD_THRESHOLD)
