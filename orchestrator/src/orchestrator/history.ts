...
export function loadHistory(): AutomationHistoryEntry[] {
    try {
        ...
        Logger.warn(`Failed to load automation history: ${err instanceof Error ? err.message : String(err)}`);
        return [];
    }
}
...
export function appendToHistory(automation: { id: string; name: string; action: AutomationAction }): void {
    try {
        ...
        Logger.warn(`Failed to append automation history: ${err instanceof Error ? err.message : String(err)}`);
    }
}
