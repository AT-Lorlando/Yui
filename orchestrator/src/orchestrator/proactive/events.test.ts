...
bad(
    { source: 's', key: 'k', kind: 'bof', importance: 'info', subject: 's' },
    /kind/,
);
bad(
    { source: 's', key: 'k', kind: 'info', importance: 'meh', subject: 's' },
    /importance/,
);
bad(
    { source: 's', key: 'k', kind: 'info', importance: 'info', subject: '' },
    /subject/,
);
bad(
    {
...
assert.strictEqual(
    parseEvents(Array(BATCH_MAX + 1).fill(e), { now: NOW }).errors[0]!.index,
    -1,
);
