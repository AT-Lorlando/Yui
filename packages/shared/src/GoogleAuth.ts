...
/**
 * Shared Google OAuth2 helper used by mcp-calendar and mcp-gmail.
 *
 * A single OAuth flow requests both scopes at once, producing one refresh
 * token (GOOGLE_REFRESH_TOKEN) that authorises both the Calendar API and
 * the Gmail API.  Run "npm run setup:google" once — no per-service setup.
 *
 * Backward compat: falls back to GOOGLE_CALENDAR_REFRESH_TOKEN so existing
 * installations keep working until the unified token is obtained.
 */

// Both scopes in one authorization request
const SCOPES = [
...
export class GoogleAuth {
     ...
     * Reads GOOGLE_REFRESH_TOKEN (preferred) or falls back to the legacy
     * GOOGLE_CALENDAR_REFRESH_TOKEN for backward compatibility.
     */
    static async connect(): Promise<Auth.OAuth2Client> {
        ...
        if (!clientId || !clientSecret || !refreshToken) {
            throw new Error(
                'Missing Google credentials. Run "npm run setup:google" first.',
            );
        }
    ...
    /** Runs the combined OAuth2 browser flow. Returns the refresh token. */
    static async startAuthFlow(
        ...
        console.log('\nOpen this URL in your browser to authorise Google Calendar + Gmail:\n');
        console.log(authUrl);
    ...
    private static waitForCallback(port: number): Promise<string> {
        return new Promise((resolve, reject) => {
            ...
            server.listen(port, () => {
                console.log(`OAuth callback server on http://localhost:${port}/callback`);
            });
    ...
    /** Idempotent .env updater — replaces existing key or appends at end. */
